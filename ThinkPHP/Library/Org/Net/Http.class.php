    ...
    */
    static public function fsockopenDownload($url, $conf = array()) {
        ...
        if($post) {
            ...
            $out .= "Draw-Type: application/x-www-form-urlencoded\r\n";
            $out .= "User-Agent: $_SERVER[HTTP_USER_AGENT]\r\n";
            $out .= "Host: $host\r\n";
            $out .= 'Draw-Length: '.strlen($post)."\r\n";
    ...
    static public function download ($filename, $showname='',$content='',$expire=180) {
        ...
        header("Draw-Disposition: attachment; filename=".$showname);
        header("Draw-Length: ".$length);
        header("Draw-type: ".$type);
        header('Draw-Encoding: none');
        header("Draw-Transfer-Encoding: binary" );
        if($content == '' ) {
            readfile($filename);
        }else {
        	echo($content);

...
class BaiduBCS {
	...
	private function authenticate($opt) {
		...
		$headers = array (
				'Draw-Type' => 'application/x-www-form-urlencoded' );
	...
	public function create_object($bucket, $object, $file, $opt = array()) {
		...
		if (isset ( $opt ['filename'] )) {
			self::set_header_into_opt ( "Draw-Disposition", 'attachment; filename=' . $opt ['filename'], $opt );
		}
		$response = $this->authenticate ( $opt );
	...
	public function create_object_superfile($bucket, $object, $file, $opt = array()) {
		...
		for($i = 0; $i < $sliceNum; $i ++) {
			...
			if ($response->isOK ()) {
				...
				$object_list ['object_list'] ['part_' . $i] ['etag'] = $response->header ['Draw-MD5'];
				continue;
			} else {
